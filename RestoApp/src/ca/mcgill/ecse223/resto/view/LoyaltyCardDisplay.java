...
public class LoyaltyCardDisplay extends SidePanel implements ActionListener {
	...
	public LoyaltyCardDisplay(Controller c, RestoAppPage p) {
		...
		//GETTING THE LIST OF CARDS FROM CONTROLLER
		List<LoyaltyCard> cards = controller.displayLoyaltyCards();
		
		for(LoyaltyCard aCard: cards) {
			Object[] cardInfo = {aCard.getEmailAddress(),aCard.getClientName(), aCard.getPhoneNumber(), aCard.getPoint()};
	...
	public void updateCardList(JButton selectedSubmit) {
		List<LoyaltyCard> cards = controller.displayLoyaltyCards();
		
		if(selectedSubmit.equals(submitRegistrationButton)) {
			LoyaltyCard justAdded = cards.get(cards.size()-1);
			Object[] cardInfo = {justAdded.getEmailAddress(),justAdded.getClientName(), justAdded.getPhoneNumber(), justAdded.getPoint()};
			cardListFrameView.addRow(cardInfo);
		}
		else if(selectedSubmit.equals(submitRemovalButton)) {
			for(int i = 0; i < cardListFrameView.getRowCount(); i++)
			{
				cardListFrameView.removeRow(i);
			}
		...
		if (e.getSource() == backButton) {
			...
			this.page.updateSidePanels();
				...
				try {
					...
					this.updateCardList(submitRegistrationButton);
			...
			try {
				...
				this.updateCardList(submitRemovalButton);
				page.updateSidePanels();
